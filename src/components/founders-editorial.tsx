...
function Portrait({
  ...
  return (
    <div className={`relative overflow-hidden bg-ocean-800 ${className}`}>
      ...
      />
      <span className="absolute bottom-4 left-4 bg-ocean-950/75 px-2.5 py-1 font-mono text-[0.65rem] tracking-[0.2em] text-aqua-300 uppercase ring-1 ring-aqua-500/25">
        {member.shortRole}
      </span>
    </div>
  );
...
function LinkedInButton({ member }: { member: TeamMember }) {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="inline-flex w-fit items-center gap-2 border border-line-strong px-4 py-2 text-sm text-chalk transition duration-300 hover:border-aqua-300/50 hover:bg-aqua-400/10 hover:text-aqua-300"
    >
...
export function FoundersEditorial({
  ...
  return (
    <div className="space-y-4">
      <Reveal>
        <article className="group grid items-stretch gap-0 border border-line bg-white/[0.02] transition duration-300 hover:border-aqua-300/35 lg:grid-cols-12">
          <Portrait
            member={lead}
          ...
          <div className="flex flex-col justify-center p-8 sm:p-12 lg:col-span-8">
            ...
            <p className="mt-2 text-sm text-aqua-300">{roleLine(lead)}</p>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {rest.map((member, index) => (
          <Reveal key={member.name} delay={(index + 1) * 90} className="h-full">
            <article className="card-lift group flex h-full flex-col overflow-hidden border border-line bg-white/[0.02] hover:border-aqua-300/35">
              <Portrait
                member={member}
              ...
              <div className="flex flex-1 flex-col p-7">
                ...
                <p className="mt-2 text-sm text-aqua-300">
                  {roleLine(member)}
                </p>
