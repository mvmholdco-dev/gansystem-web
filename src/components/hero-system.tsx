...
export function SignalRail() {
  ...
  return (
    <figure
      className="relative border border-line bg-ink-950/60 px-5 py-6 sm:px-7"
      role="group"
    ...
    >
      ...
      <div className="relative mt-5">
        <span
          aria-hidden="true"
          className="absolute left-0 right-0 top-[22px] hidden h-px bg-gradient-to-r from-tide-400/50 via-leaf-500/50 to-leaf-500/25 lg:block"
        />
          ...
          {stages.map((stage, index) => {
            ...
            return (
              <li key={stage.label} className="flex items-start gap-3 lg:block">
                <span
                  aria-hidden="true"
                  className="signal-node flex h-11 w-11 shrink-0 items-center justify-center border border-line-strong bg-ink-950 text-leaf-400"
