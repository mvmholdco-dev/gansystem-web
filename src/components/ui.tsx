...
export function SectionHeading({
  ...
  const heading = (
    <h2 className={`mt-5 font-display text-[1.875rem] leading-[1.1] font-semibold tracking-[-0.02em] sm:text-[2.5rem] ${darkText ? 'text-sand-950' : 'text-chalk'}`}>
      {title}
    </h2>
  );
    ...
    return (
      <Reveal className="grid gap-x-12 gap-y-6 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] lg:items-end">
        ...
        <div className="lg:pb-1">
          {lead ? (
            <p className={`text-[1.0625rem] leading-[1.8] sm:text-[1.125rem] ${darkText ? 'text-sand-950' : 'text-fog'}`}>
              {lead}
            </p>
          ) : null}
  ...
  return (
    ...
    >
      ...
      {heading}
      {lead ? (
        <p className={`mt-5 text-[1.0625rem] leading-[1.8] sm:text-[1.125rem] ${darkText ? 'text-sand-950' : 'text-fog'}`}>
          {lead}
        </p>
      ) : null}
    </Reveal>
